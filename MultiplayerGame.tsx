...
export function MultiplayerGame({
  ...
  const [opponentGameOver, setOpponentGameOver] = useState(false);
    // --- MODERN DAS/ARR ENGINE SETTINGS ---
  const dasDelay = 170; // Time in milliseconds before auto-repeat kicks in (Standard Tetrio: ~140-180ms)
  const arrRate = 30;   // Time in milliseconds between consecutive fast moves (Standard Tetrio: ~10-40ms)
  ...
  const softDropIntervalRef = useRef<ReturnType<typeof setInterval> | null>(null);
    // --- TOURAMENT LOCK DELAY CONFIGURATION ---
  const lockDelayLimit = 500; // Grace period in milliseconds before a piece locks onto the floor
  const maxLockResets = 15;   // Maximum number of spins/moves allowed to refresh the grace period timer
  const lockTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const lockResetCountRef = useRef<number>(0);

  const stateRef = useRef(gameState);

    // Helper to permanently bake a piece onto the grid rows and cycle the bag queue
    ...
    setGameState((prev) => {
      ...
      // --- ADVANCED PRO STREAK CALCULATIONS ---
      const isLineClear = cleared > 0;
      const currentCombo = isLineClear ? (prev.combo || 0) + 1 : 0;

      // Spin Verification: Was it a T-Piece that executed an SRS wall kick when it landed?
      ...
      if (isLineClear) {
        // B2B continues if it's a Tetris or any T-Spin clear. It snaps if it's a basic single/double/triple.
        currentB2B = isPowerClear ? true : false;
      }

      // Modern competitive garbage attack calculation
      let garbageToSend = calculateGarbageSent(cleared);
      if (isTSpin && isLineClear) {
        garbageToSend += cleared * 2; // Add massive T-Spin spike bonus!
      }
      if (currentCombo > 1) {
        garbageToSend += Math.floor((currentCombo - 1) / 2); // Add combo scaling
      }
      if (currentB2B && isPowerClear && prev.backToBack) {
        garbageToSend += 1; // Add B2B chain bonus spike row!
      }
  ...
  // Handle receiving garbage rows sent from opponent attacks
  useEffect(() => {
    ...
    // Core Animation Frame Gravity Loop with Grace Lock Delay
  useEffect(() => {
    ...
    const gameTick = (time = 0) => {
      ...
      if (dropCounter > Math.max(50, 1000 - (stateRef.current.level * 80))) {
        ...
        if (nextMove) {
          setGameState((prev) => ({ ...prev, currentPiece: nextMove }));
          onSendMessage({ type: "gameState", playerId, data: { ...stateRef.current, currentPiece: nextMove } });
          
          // Clear lock countdown if the piece becomes free to move down again
          if (lockTimeoutRef.current) {
            clearTimeout(lockTimeoutRef.current);
            lockTimeoutRef.current = null;
          }
          ...
          // --- LOCK DELAY TRIGGERS ---
          // If the piece hits the floor, start the countdown instead of locking instantly
          if (!lockTimeoutRef.current) {
   ...
   // Handle Keyboard Interactivity Inputs with page-lock, DAS/ARR Engine, and Rapid Soft Drop
  useEffect(() => {
    ...
    // Fast Soft Drop trigger function
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // --- MODERN RAPID SOFT DROP INJECTION ---
      if (e.key === "ArrowDown" && !softDropIntervalRef.current) {
        // Instantly drop 1 space on click
        triggerSoftDropMove();
        // Stream the piece downward every 15ms while held (Ultra-fast soft drop factor!)
        softDropIntervalRef.current = setInterval(() => {
          triggerSoftDropMove();
        }, 15);
      }
             else if (e.key === "ArrowUp") {
        // Clockwise Rotation with full SRS position extraction mapping
        ...
        // --- FIXED: RESTORED CALCULATION ENGINE LINES ---
        ...
        // Update state with pro streak calculation math
        setGameState((prev) => {
          let workingBag = [...(prev.bag || [])];
          if (workingBag.length < 5) workingBag = [...workingBag, ...generateShuffledBag()];
          ...
          lastWasSpinRef.current = false; // Reset spin turn index
    ...
    const handleKeyUp = (e: KeyboardEvent) => {
      ...
      // Kill the downward repeat speed loop as soon as ArrowDown is lifted
      if (e.key === "ArrowDown") {
        clearSoftDropTimer();
      }
    };
  ...
  return (
    <div className="flex flex-col items-center justify-center p-4">
      ...
      <div className="flex-row-layout" style={{ maxWidth: '1200px', backgroundColor: '#030712', padding: '24px', borderRadius: '16px', border: '1px solid #1f2937', margin: '0 auto' }}>
                ...
                {/* LEFT COMPONENT COLUMN: HOLD PANEL ITEM DOCK */}
        ...
        {/* CENTER COMPONENT COLUMN: MAIN GAME PLAYING BOARD */}
        <div className="flex flex-col items-center">
          ...
          <div className="mt-4 grid grid-cols-3 gap-4 text-center bg-gray-800 p-3 rounded-lg w-full">
            ...
            <div><p className="text-xs text-gray-400">Level</p><p className="text-lg font-bold text-cyan-400">{gameState.level}</p></div>
                ...
                {/* RIGHT COMPONENT COLUMN: MODERN 5-PIECE PREVIEW WINDOW QUEUE */}
        <div className="sidebar-panel">
          ...
          <div className="next-queue-stack">
            {Array.isArray(gameState.nextPiece) && gameState.nextPiece.map((pieceId, index) => (
      ...
      {!gameState.gameOver && !opponentGameOver && (
        <button
          onClick={() => setIsGameActive((prev) => !prev)}
