...
// Force the Multiplayer Game interface board to execute ONLY on the client browser
const MultiplayerGame = dynamic(
  () => import("@/MultiplayerGame").then((mod) => mod.MultiplayerGame),
  { ssr: false }
);
...
export default function Home() {
  ...
  // Stop random values from evaluating on server build passes
  useEffect(() => {
    ...
    try {
      ...
      client.on("sendGarbage", (message) => {
        if (message.playerId !== playerId && message.garbageLines) {
          ...
          console.log(`Received ${message.garbageLines} garbage lines from opponent`);
        }
      });
